...
main(void)
{
    const uint32_t lhs[ELEM_COUNT] = {
        ...
        npu_float_to_bits(10.0f),
        npu_float_to_bits(11.0f),
    };
    ...
    npu_golden_vpu_elemwise_f32(VPU_OP_VSUB, lhs, rhs, expected, ELEM_COUNT);
    ...
    if (npu_wait_float_vector_close(
        ...
        printf("VPU_ELEMWISE_SUB_F32_FAIL\n");
        npu_expect_float_vector_close("VPU_ELEMWISE_SUB_F32", expected,
                                      actual, ELEM_COUNT, 0.03f, 0.03f);
        return 1;
    }

    printf("VPU_ELEMWISE_SUB_F32_PASS\n");
    return 0;
}
