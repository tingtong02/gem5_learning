...
enum ElemwiseMulI32Layout
{
    ...
    DST_PORT = 2U,
    ELEM_COUNT = 4U,
    SYNC_INDICATOR = 0x43U,
};

int
main(void)
{
    const uint32_t lhs[ELEM_COUNT] = {2U, 3U, 4U, 5U};
    const uint32_t rhs[ELEM_COUNT] = {6U, 7U, 8U, 9U};
    ...
    npu_golden_vpu_elemwise_i32(VPU_OP_VMUL, lhs, rhs, expected, ELEM_COUNT);
    ...
    if (npu_wait_u32_vector_match(NULL,
        ...
        printf("VPU_ELEMWISE_MUL_I32_FAIL\n");
        npu_expect_u32_vector("VPU_ELEMWISE_MUL_I32", expected, actual,
                              ELEM_COUNT);
        return 1;
    }

    printf("VPU_ELEMWISE_MUL_I32_PASS\n");
    return 0;
}
