...
main(void)
{
    ...
    vpu_cmd_launch_ternary(VPU_DEVICE_ID, VPU_OP_VFMA, SYNC_INDICATOR, 0x7U,
                           0x1U, 1U, ELEM_COUNT, sizeof(uint32_t),
                           sizeof(uint32_t), VPU_DATA_F32);

    if (npu_wait_float_vector_close(
            NULL, npu_spm_slot_word_ptr_default(SRC0_PORT), expected,
            ELEM_COUNT, 60000000ULL, 0.03f, 0.03f) != 0) {
        npu_spm_load_u32_vector(SRC0_PORT, actual, ELEM_COUNT);
        printf("VPU_FMA_INPLACE_FAIL\n");
        npu_expect_float_vector_close("VPU_FMA_INPLACE", expected, actual,
                                      ELEM_COUNT, 0.03f, 0.03f);
        return 1;
    }

    printf("VPU_FMA_INPLACE_PASS\n");
    return 0;
}
