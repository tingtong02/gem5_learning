...
binary = resolve_binary_path(__file__, "vpu_legacy_dual_device_basic_riscv")


register_npu_test(
    NpuRunnerSpec(
        name="vpu_legacy_dual_device_basic",
        config=resolve_config_path(__file__),
        config_args=tuple(make_binary_config_args(binary)),
        gem5_args=("--debug-flags=VPU",),
        verifier_specs=r"VPU_LEGACY_DUAL_DEVICE_BASIC_PASS",
