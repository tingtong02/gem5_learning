...
EXPECTED_RESULT = {
    ...
    "lut_requests": 4,
    "lut_commands": 1,
    "latencies_match": True,
    "completion_ticks_match": True,
}
...
def collect_simulation_result(builder, exit_cause, exit_code):
    ...
    softmax_latency = vpu.lastSoftmaxExecuteLatency()
    lut_completion_tick = vpu.lastLutCompletionTick()
    softmax_completion_tick = vpu.lastSoftmaxCompletionTick()
    return {
        ...
        "latencies_match": lut_latency == softmax_latency,
        "completion_ticks_match": (
            lut_completion_tick == softmax_completion_tick
        ),
    }
...
for key, value in result.items():
    print(f"VPU_SOFTMAX_RAMP_{key.upper()}={value}")

if verify_simulation_result(result):
    print("VPU_SOFTMAX_RAMP_PASS")
