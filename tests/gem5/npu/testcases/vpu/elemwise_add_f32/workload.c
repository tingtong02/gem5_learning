...
main(void)
{
    ...
    npu_golden_vpu_elemwise_f32(VPU_OP_VADD, lhs, rhs, expected, ELEM_COUNT);

    vpu_cmd_launch_binary(VPU_DEVICE_ID, VPU_OP_VADD, SYNC_INDICATOR, 0x3U,
    ...
    if (npu_wait_float_vector_close(
        ...
        printf("VPU_ELEMWISE_ADD_F32_FAIL\n");
        npu_expect_float_vector_close("VPU_ELEMWISE_ADD_F32", expected,
                                      actual, ELEM_COUNT, 0.03f, 0.03f);
        return 1;
    }

    printf("VPU_ELEMWISE_ADD_F32_PASS\n");
    return 0;
}
