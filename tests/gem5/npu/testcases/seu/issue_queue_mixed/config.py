...
def verify(result):
    return (
        ...
        and result["completed_cmds"] == 3
        and result["prologues"] == 3
        and result["executes"] == 4
        and result["epilogues"] == 3
        and result["iterations"] == 3
        and result["read_resps"] == 8192
        and result["write_resps"] == 8192
        and result["max_active_micro_ops"] >= 3
    )
...
emit_summary("SEU_ISSUE_QUEUE_MIXED", result)

if verify(result):
    print("SEU_ISSUE_QUEUE_MIXED_PASS")
